...
public class MusicActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void updateSongInfo() {
        int currentIndex = musicControl.getCurrentSongIndex(); // 从服务中获取当前歌曲的索引
        name_song.setText(frag1.name[currentIndex]); // 更新歌曲名称文本视图
        ImageView iv_music = findViewById(R.id.iv_music); // 获取音乐图标的 ImageView
        iv_music.setImageResource(frag1.icons[currentIndex]); // 更新音乐图标
    }
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    @Override
    public void onClick(View v) {
        ...
        if (viewId == R.id.btn_play) {
            ...
            animator.pause();
        } else if (viewId == R.id.btn_continue_play) {
